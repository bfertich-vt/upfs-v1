...
export const evidenceFor = (plan = syntheticPlan) => plan.scenarios.map((scenario, i) => ({
  scenario, status: 'passed', synthetic_only: true, deployment: 'not-performed',
  observed: { tenant_escape: 0, p95_ms: 420 + i * 10, lag_minutes: 4 + i, rpo_minutes: 5 + i, rto_minutes: 20 + i, corrective_forward: true },
  evidence_ref: `sha256:${sha256({ scenario, version: plan.version, i })}`
}));
...
export function evaluateResilience({ plan = syntheticPlan, infrastructure = {}, evidence = evidenceFor(plan) } = {}) {
  ...
  const refs = Array.isArray(evidence) && evidence.length === required.length && evidence.every((e) => typeof e.evidence_ref === 'string' && /^sha256:[a-f0-9]{64}$/.test(e.evidence_ref));
  ...
  const thresholds = (evidence || []).every((e) => e.observed && e.observed.tenant_escape === 0 && e.observed.p95_ms <= obj.read_p95_ms && e.observed.lag_minutes <= obj.projection_lag_minutes && e.observed.rpo_minutes <= obj.rpo_minutes && e.observed.rto_minutes <= obj.rto_minutes && e.observed.corrective_forward === true);
