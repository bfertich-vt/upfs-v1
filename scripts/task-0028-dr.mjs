...
export function evaluateDR({ contract = syntheticContract, infrastructure = {} } = {}) {
  ...
  check(checks, 'tenant-routing', Array.isArray(c.tenant_routing) && c.tenant_routing.length > 0 && c.tenant_routing.every((r) => r.tenant_id && regions.some((x) => x.id === r.region) && cells.some((x) => x.id === r.cell && x.region === r.region) && Number.isInteger(r.version) && r.version > 0), 'tenant routes must be positive-versioned and region/cell scoped');
  check(checks, 'evacuation', c.evacuation?.owner && c.evacuation?.approval === 'dual-control' && c.evacuation?.procedure && Number.isFinite(c.evacuation.max_minutes) && c.evacuation.max_minutes > 0, 'evacuation must be owned, approved, and positively bounded');
  const backupIds = new Set((c.backups || []).map((b) => b.id));
  check(checks, 'backup-restore', Array.isArray(c.backups) && c.backups.length > 0 && c.backups.every((b) => b.id && b.target && b.encrypted === true && b.retention_days > 0 && regions.some((r) => r.id === b.region)) && Array.isArray(c.restore_points) && c.restore_points.every((p) => p.backup_id && backupIds.has(p.backup_id) && regions.some((r) => r.id === p.region) && p.verified === true), 'encrypted backup targets and verified restore points must reference known backups and regions');
  check(checks, 'failover-owner', c.failover?.owner && regions.some((r) => r.id === c.failover.target_region) && c.failover?.approval === 'dual-control' && c.failover.corrective_forward === true, 'failover ownership, known target, dual control, and corrective-forward required');
  ...
  const evidenceOk = evidence.synthetic_only === true && evidence.deployment === 'not-performed' && Number.isFinite(evidence.backup_lag_minutes) && evidence.backup_lag_minutes >= 0 && Number.isFinite(evidence.failover_minutes) && evidence.failover_minutes >= 0 && evidence.backup_lag_minutes <= c.objectives?.rpo_minutes && evidence.failover_minutes <= c.objectives?.rto_minutes && evidence.corrective_forward === true;
