...
export function evaluateInfrastructurePolicy({ candidate = manifest, readinessContract = contract } = {}) {
  ...
  const cr = candidate?.cells_regions || {};
  const envRegions = new Set(Object.values(envs).map(e => e.region).filter(Boolean));
  const envCells = Object.values(envs).map(e => e.cell).filter(Boolean);
  check(checks, 'cells-regions', cr.regions_required >= readinessContract.cells_regions.regions_required && cr.cells_per_region_required >= readinessContract.cells_regions.cells_per_region_required && cr.routing_managed === true && cr.failover_managed === true && typeof cr.failover_target === 'string' && cr.failover_target.length > 0 && envRegions.size >= cr.regions_required && new Set(envCells).size >= cr.cells_per_region_required, 'region/cell cardinality, managed routing, and explicit failover target required');
