...
test("TASK-0002 protected-review activation is exact and fails closed", (t) => {
  ...
  const later = queue.replace(
    /(id: TASK-0008[\s\S]*?status:) blocked/,
    "$1 complete",
  );
  expectInvalid(root, later, /TASK-0008\.json cannot be resolved/);
});
...
test("TASK-0004 activation binds exact evidence, status, limitations, rejections, and later-task freeze", (t) => {
  const source = process.cwd();
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "upfs-task-0004-activation-"),
  );
  execFileSync("git", ["clone", "--shared", source, root], { stdio: "ignore" });
  execFileSync("git", ["checkout", "--detach", "HEAD"], {
    cwd: root,
  ...
  const closure = JSON.parse(fs.readFileSync(closurePath, "utf8"));
  assert.doesNotThrow(() => validateQueueDocument(queue, root));
  const mutate = (change, pattern = /exact TASK-0004 protected evidence/) => {
    const candidate = structuredClone(closure);
    change(candidate);
    fs.writeFileSync(closurePath, `${JSON.stringify(candidate, null, 2)}\n`);
    expectInvalid(root, queue, pattern);
  ...
  for (const substitution of [
    ...
    (r) => (r.protected_merge.commit = "a".repeat(40)),
    (r) => (r.protected_merge.head_tree = "a".repeat(40)),
    (r) => (r.protected_merge.merged_at = "2026-08-07T20:17:38-04:00"),
    (r) => (r.hosted.post_merge_checks[0].run_id = 1),
    (r) => (r.rejected_candidates[0] = "a".repeat(40)),
    (r) => r.rejected_candidates.pop(),
  ])
  ...
  fs.writeFileSync(
    ...
    matrix.replace(
      /\| TASK-0004 ([^\n]*?)\| Proven reference implementation \| ACCEPTED \|/,
      "| TASK-0004 $1| Proven production implementation | ACCEPTED |",
    ),
  );
  expectInvalid(
    root,
  ...
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0004[\s\S]*?status:) complete/, "$1 blocked"),
    /TASK-0004 has a closure record but is not complete/,
  );
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0008[\s\S]*?status:) blocked/, "$1 complete"),
    /TASK-0008\.json cannot be resolved/,
  );
});

test("TASK-0005 activation binds exact protected evidence and fails closed", (t) => {
  const source = process.cwd();
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "upfs-task-0005-activation-"),
  );
  execFileSync("git", ["clone", "--shared", source, root], { stdio: "ignore" });
  execFileSync("git", ["checkout", "--detach", "HEAD"], {
    cwd: root,
  ...
  assert.doesNotThrow(() => validateQueueDocument(queue, root));
  let mutations = 0;
  const mutate = (change, pattern = /exact TASK-0005 protected evidence/) => {
    const candidate = structuredClone(closure);
    change(candidate);
    fs.writeFileSync(closurePath, `${JSON.stringify(candidate, null, 2)}\n`);
    expectInvalid(root, queue, pattern);
  ...
  for (const substitution of [
    ...
    (r) => (r.protected_merge.commit = "a".repeat(40)),
    (r) => (r.protected_merge.head_tree = "a".repeat(40)),
    (r) => (r.protected_merge.merged_at = "2026-08-07T22:19:16-04:00"),
    (r) => (r.hosted.post_merge_checks[0].run_id = 1),
    (r) => (r.protected_review.reviewer_role = "Backend"),
  ])
    mutate(substitution);
  mutate(
    (r) => r.limitations.splice(0),
  ...
  assert.equal(mutations, 18);
  fs.writeFileSync(
    matrixPath,
    matrix.replace(
      /\| TASK-0005 ([^\n]*?)\| Proven reference implementation \| ACCEPTED \|/,
      "| TASK-0005 $1| Proven production implementation | ACCEPTED |",
    ),
  );
  expectInvalid(
    root,
  ...
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0005[\s\S]*?status:) complete/, "$1 blocked"),
    /TASK-0005 has a closure record but is not complete/,
  );
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0008[\s\S]*?status:) blocked/, "$1 complete"),
...
test("TASK-0006 activation binds accepted, rejected, hosted, and limitation evidence exactly", (t) => {
  const source = process.cwd();
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "upfs-task-0006-activation-"),
  );
  execFileSync("git", ["clone", "--shared", source, root], { stdio: "ignore" });
  execFileSync("git", ["checkout", "--detach", "HEAD"], {
    cwd: root,
  ...
  assert.doesNotThrow(() => validateQueueDocument(queue, root));
  let mutations = 0;
  const mutate = (change, pattern = /exact TASK-0006 protected evidence/) => {
    const candidate = structuredClone(closure);
    change(candidate);
    fs.writeFileSync(closurePath, `${JSON.stringify(candidate, null, 2)}\n`);
    expectInvalid(root, queue, pattern);
  ...
  const substitutions = [
    ...
    (r) => (r.rejected_reviews[0].review = "docs/reviews/forged-r1.md"),
    (r) => (r.rejected_reviews[0].review_sha256 = "a".repeat(64)),
    (r) => (r.rejected_reviews[0].review_commit = "a".repeat(40)),
    (r) => (r.rejected_reviews[1].candidate_commit = "b".repeat(40)),
    (r) => (r.rejected_reviews[1].review = "docs/reviews/forged-r2.md"),
    (r) => (r.rejected_reviews[1].review_sha256 = "b".repeat(64)),
    (r) => (r.rejected_reviews[1].review_commit = "b".repeat(40)),
    (r) => r.rejected_reviews.pop(),
    (r) => r.rejected_reviews.push(structuredClone(r.rejected_reviews[0])),
    (r) => r.rejected_reviews.reverse(),
    (r) => (r.protected_review.mechanism = "native-human-review"),
    ...
    (r) => (r.hosted.post_merge_checks[0].name = "forged-validation"),
    (r) => (r.hosted.post_merge_checks[0].run_id = 1),
    (r) => (r.hosted.post_merge_checks[0].job_id = 1),
    (r) => (r.hosted.post_merge_checks[0].head_sha = "a".repeat(40)),
    (r) => (r.hosted.post_merge_checks[0].conclusion = "failure"),
    (r) => (r.hosted.post_merge_checks[1].name = "forged-security"),
    (r) => (r.hosted.post_merge_checks[1].run_id = 1),
    (r) => (r.hosted.post_merge_checks[1].job_id = 1),
    (r) => (r.hosted.post_merge_checks[1].head_sha = "a".repeat(40)),
    (r) => (r.hosted.post_merge_checks[1].conclusion = "failure"),
    (r) => r.hosted.post_merge_checks.pop(),
    (r) => (r.protected_merge.commit = "a".repeat(40)),
    (r) => (r.protected_merge.base_parent = "a".repeat(40)),
    (r) => (r.protected_merge.head_tree = "a".repeat(40)),
    (r) => (r.protected_merge.merged_at = "2026-08-08T02:12:49-04:00"),
    (r) => (r.protected_merge.pull_request = 1),
  ];
  for (const substitution of substitutions) mutate(substitution);
  assert.equal(mutations, 64);
  ...
  fs.writeFileSync(
    ...
    matrix.replace(
      /\| TASK-0006 ([^\n]*?)\| Proven reference implementation \| ACCEPTED \|/,
      "| TASK-0006 $1| Proven production implementation | ACCEPTED |",
    ),
  );
  expectInvalid(
    root,
  ...
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0006[\s\S]*?status:) complete/, "$1 blocked"),
    /TASK-0006 has a closure record but is not complete/,
  );
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0008[\s\S]*?status:) blocked/, "$1 complete"),
...
test("TASK-0007 activation fails closed on evidence, status, classification, and dependency drift", (t) => {
  const source = process.cwd();
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "upfs-task-0007-activation-"),
  );
  execFileSync("git", ["clone", "--shared", source, root], { stdio: "ignore" });
  execFileSync("git", ["checkout", "--detach", "HEAD"], {
    cwd: root,
  ...
  assert.doesNotThrow(() => validateQueueDocument(queue, root));
  let mutations = 0;
  const mutate = (change, pattern = /exact TASK-0007 protected evidence/) => {
    const candidate = structuredClone(closure);
    change(candidate);
    fs.writeFileSync(closurePath, `${JSON.stringify(candidate, null, 2)}\n`);
    expectInvalid(root, queue, pattern);
  ...
  const substitutions = [
    ...
    (r) => (r.independent_qa.reviewed_candidate = "a".repeat(40)),
    (r) => (r.rejected_reviews[0].candidate_commit = "a".repeat(40)),
    (r) => (r.rejected_reviews[0].review = "docs/reviews/forged-r2.md"),
    (r) => (r.rejected_reviews[0].review_sha256 = "a".repeat(64)),
    (r) => (r.rejected_reviews[0].review_commit = "a".repeat(40)),
    (r) => r.rejected_reviews.pop(),
    (r) => r.rejected_reviews.push(structuredClone(r.rejected_reviews[0])),
    ...
    (r) => (r.protected_merge.base_parent = "a".repeat(40)),
    (r) => (r.protected_merge.head_tree = "a".repeat(40)),
    (r) => (r.protected_merge.merged_at = "2026-08-08T08:02:29-04:00"),
    (r) => (r.protected_merge.pull_request = 1),
  ];
  for (const substitution of substitutions) mutate(substitution);
  assert.equal(mutations, 59);
  ...
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0007[\s\S]*?status:) complete/, "$1 blocked"),
    /TASK-0007 has a closure record but is not complete/,
  );
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0009[\s\S]*?status:) blocked/, "$1 complete"),
    /TASK-0009\.json cannot be resolved/,
  );
  fs.rmSync(closurePath);
  expectInvalid(root, queue, /TASK-0007\.json cannot be resolved/);
});

test("TASK-0008 activation fails closed on evidence, status, classification, and dependency drift", (t) => {
  const source = process.cwd();
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "upfs-task-0008-activation-"),
  );
  execFileSync("git", ["clone", "--shared", source, root], { stdio: "ignore" });
  execFileSync("git", ["checkout", "--detach", "HEAD"], {
    cwd: root,
  ...
  assert.doesNotThrow(() => validateQueueDocument(queue, root));
  let mutations = 0;
  const mutate = (change, pattern = /exact TASK-0008 protected evidence/) => {
    const candidate = structuredClone(closure);
    change(candidate);
    fs.writeFileSync(closurePath, `${JSON.stringify(candidate, null, 2)}\n`);
    expectInvalid(root, queue, pattern);
  ...
  const substitutions = [
    ...
    (r) => (r.rejected_reviews[0].review_sha256 = "a".repeat(64)),
    (r) => (r.rejected_reviews[0].review_commit = "a".repeat(40)),
    (r) => (r.rejected_reviews[0].preservation_commit = "a".repeat(40)),
    (r) => r.rejected_reviews.pop(),
    (r) => r.rejected_reviews.push(structuredClone(r.rejected_reviews[0])),
    (r) => (r.protected_review.mechanism = "emergency-bypass"),
    (r) => (r.protected_review.candidate_commit = "a".repeat(40)),
  ...
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0008[\s\S]*?status:) complete/, "$1 blocked"),
    /TASK-0008 has a closure record but is not complete/,
  );
  expectInvalid(
    root,
    queue.replace(/(id: TASK-0009[\s\S]*?status:) blocked/, "$1 complete"),
