...
export function createSearchWorkbench({ root, api, limit = DEFAULT_LIMIT, onState = () => {} } = {}) {
  ...
  const state = { status: 'idle', query: '', data: [], nextCursor: null, requestId: null, watermark: 0, consistency: null, error: null, loadingMore: false };
  let requestSerial = 0;
  ...
  const search = async (query = state.query, cursor = null) => {
    ...
    if (!normalized || normalized.length > MAX_QUERY) { setState({ status: 'error', error: { code: 'invalid_query', message: 'Enter a search term (1–500 characters).' } }); notify(); return; }
    const serial = ++requestSerial;
    setState({ status: cursor ? state.status : 'loading', loadingMore: Boolean(cursor), query: normalized, error: null }); notify();
    try {
      const result = await api.search({ query: normalized, limit, cursor });
      if (serial !== requestSerial) return;
      setState({ status: result.data?.length ? 'success' : 'empty', loadingMore: false, data: cursor ? [...state.data, ...(result.data || [])] : (result.data || []), nextCursor: result.page?.next_cursor ?? null, requestId: result.request_id ?? null, watermark: result.watermark ?? 0, consistency: result.consistency ?? null, error: null }); notify();
    } catch (error) {
      if (serial !== requestSerial) return;
      setState({ status: 'forbidden' === error.code || error.status === 403 ? 'forbidden' : 'error', loadingMore: false, error: { code: error.code, status: error.status, message: error.status === 403 ? 'You do not have permission to search these transactions.' : 'Search failed. Try again.' } }); notify();
    }
  };
  const render = () => {
    ...
    const button = document.createElement('button'); button.type = 'submit'; button.textContent = 'Search'; form.append(input, button); root.append(form);
    ...
    if (state.status === 'error' && state.query) { const retry = document.createElement('button'); retry.type = 'button'; retry.textContent = 'Retry'; retry.addEventListener('click', () => search(state.query)); root.append(retry); }
  };
  render();
  return { search, snapshot, cancel: () => { requestSerial += 1; } };
}
