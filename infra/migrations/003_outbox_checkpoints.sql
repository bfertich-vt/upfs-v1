...
CREATE TABLE IF NOT EXISTS transactional_outbox (
  id text PRIMARY KEY, tenant_id uuid NOT NULL REFERENCES tenants(id), aggregate_id text,
...
CREATE POLICY transactional_outbox_tenant ON transactional_outbox USING (tenant_id::text = current_setting('app.tenant_id', true)) WITH CHECK (tenant_id::text = current_setting('app.tenant_id', true));
CREATE TABLE IF NOT EXISTS durable_checkpoints (
  tenant_id uuid NOT NULL REFERENCES tenants(id), consumer text NOT NULL, sequence bigint NOT NULL DEFAULT 0 CHECK (sequence >= 0), updated_at timestamptz NOT NULL DEFAULT now(), PRIMARY KEY (tenant_id,consumer)
);
...
CREATE POLICY durable_checkpoints_tenant ON durable_checkpoints USING (tenant_id::text = current_setting('app.tenant_id', true)) WITH CHECK (tenant_id::text = current_setting('app.tenant_id', true));
