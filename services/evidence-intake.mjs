...
export class RawEvidenceIntakeService {
  ...
  async intake({ actor, tenantId, environmentId, evidence, idempotencyKey }) {
    ...
    if (prior) return prior.payload_hash === payloadHash ? prior.result : this.#error(409, 'idempotency_conflict');
    ...
    const record = Object.freeze({ ...evidence, content: evidence.content, status: 'quarantined', classification: safeClassification, observed_at: evidence.observed_at, ingested_at: this.now().toISOString() });
    const result = { status: 201, body: { ...record }, headers: { etag: `"${evidence.id}:${evidence.content_hash}"` } };
...
export function validateEvidence(evidence) {
  ...
  if (!/^[a-f0-9]{64}$/.test(evidence.content_hash) || !isValidDateTime(evidence.observed_at)) return 'invalid_evidence';
