...
export class TransactionProjectionService {
  ...
  rebuild({ actor, tenantId, canonicalTransactions = [], watermark = 0 }) {
    ...
    const records = canonicalTransactions.filter((item) => item?.tenant_id === tenantId);
  ...
  search({ actor, tenantId, query = '', limit = 25, cursor = null }) {
    ...
    let offset = 0;
    if (cursor) {
      try { const decoded = JSON.parse(Buffer.from(cursor, 'base64url').toString()); const { mac, ...payload } = decoded; if (decoded.tenantId !== tenantId || decoded.queryHash !== queryHash || !Number.isInteger(decoded.offset) || decoded.offset < 0 || !mac || !safeMac(payload, mac, this.#cursorKey)) throw new Error(); offset = decoded.offset; } catch { return error(400, 'invalid_cursor'); }
    }
    ...
    const cursorPayload = { tenantId, queryHash, offset: nextOffset };
